...
verificar:tokens  → 48 arquivos varridos em src/, sendo 1 de token
...
```
[largura-360]  Alvos de toque medidos: 24 · abaixo de 44 px: 0
[largura-390]  Alvos de toque medidos: 24 · abaixo de 44 px: 0
[largura-430]  Alvos de toque medidos: 24 · abaixo de 44 px: 0
[largura-480]  Alvos de toque medidos: 24 · abaixo de 44 px: 0
[largura-768]  Alvos de toque medidos: 24 · abaixo de 44 px: 0
[largura-1024] Alvos de toque medidos: 24 · abaixo de 44 px: 0
[largura-1280] Alvos de toque medidos: 24 · abaixo de 44 px: 0
```

**O contador é o que faz o verde significar alguma coisa**: 24 elementos medidos, não 0.

Eram **20** antes da correção da Parada 3: a vitrine mostrava 9 das 12 células da matriz do botão.
Faltavam o **ícone desabilitado** e a **largura total** no secundário, no fantasma e no ícone. O
contador estava honesto — media o que a página tinha —, mas ele impede "verde sem medir nada", não
"medi tudo de uma página que não mostra tudo".

**Matriz do Botao agora completa**: 4 variantes × 3 estados = **12 células**, num arquivo só
(`BotaoDemo.tsx`), onde a conferência é contar linhas × colunas.
...
```
Elementos com foco verificado: 18 · sem foco visível: 0
```
...
**Evidência 10 — tokens**: `verificar:tokens` com 48 arquivos varridos e zero valores à mão.
...
## Correções da Parada 3 — aplicadas em 21/08/2026

### Correção 1 — a vitrine mostrava 9 das 12 células da matriz do botão

Faltavam **ícone desabilitado** e **largura total** no secundário, no fantasma e no ícone. Os dois
que pesavam:

- o **ícone desabilitado** é o único botão sem texto, e por isso o estado que mais precisa ser
  olhado. Medido: traço `--color-accent-700` a 45% sobre `--color-bg` dá **2,10:1** — entre o
  primário (1,86:1) e o secundário (2,72:1). Isento do critério 1.4.3 e aceito conscientemente.
  Registrado no FIDELIDADE e na §3 do ADR-0004;
- a **largura total combinada** era a única prova possível da justificativa da booleana
  `larguraTotal` no contrato. Provada uma vez só, no primário, ela ficava afirmada e não
  demonstrada.

Efeito medido: alvos de toque de **20 para 24**, foco verificado de **15 para 18**.

### Correção 2 — `page.tsx` tinha 213 linhas contra o limite de 150

Escolhida a segunda saída: **um arquivo de demonstração por componente**, montados pela `page.tsx`.

| Arquivo | Linhas |
| --- | --- |
| `page.tsx` (só compõe) | 53 |
| `BotaoDemo.tsx` | 54 |
| `CartaoDemo.tsx` | 55 |
| `CampoDemo.tsx` | 45 |
| `Secao.tsx` | 35 |
| `LinhaDeInscricaoDemo.tsx` | 30 |
| `EstadoVazioDemo.tsx` | 25 |
| `EtiquetaDemo.tsx` | 21 |
| `SeparadorDemo.tsx` | 20 |
| `IconeDemo.tsx` | 13 |

Todos abaixo de 150, **sem exceção nova nos padrões**. A regra de crescimento fica mecânica:
componente novo acrescenta arquivo em vez de engordar uma página que só cresce, e a matriz de
estados do botão passa a ter um lugar único onde viver.

### Duas pendências nomeadas, que antes eram só linha de fidelidade

- **`docs/ADR-0003` §4.2** — o sistema não tem cor semântica de erro, sucesso ou aviso. O `Campo`
  resolveu com o que existe, mas a primeira feature com formulário de verdade vai esbarrar nisso.
  As três opções estão escritas; a decisão não foi tomada.
- **`docs/PADROES-DE-CODIGO.md` §3** — o verificador de tokens não distingue valor de estilo de
  texto de tela em `.tsx`. A saída é escrever a medida em prosa com espaço (`16 px`), que não colide
  e é tipograficamente correta.
